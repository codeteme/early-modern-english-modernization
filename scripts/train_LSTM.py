...
class SpellingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_seq = [self.target_vocab['<START>']] + \
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, embedding_dim, hidden_dim, n_layers=2, dropout=0.3):
        ...
        self.lstm = nn.LSTM(embedding_dim, hidden_dim, n_layers,
                           ...
                           bidirectional=False)  # Keep unidirectional for memory
...
class SpellingNormalizer:
    ...
    def prepare_data(self, csv_path, test_size=0.15, val_size=0.15, augment=True):
        ...
        if augment:
            ...
            import random
            for old, modern in zip(input_texts[:100], target_texts[:100]):  # Limit to avoid too much noise
        ...
        self.input_vocab = {char: idx + 1 for idx, char in enumerate(sorted(input_chars))}
        self.input_vocab['<PAD>'] = 0
        
        self.target_vocab = {char: idx + 3 for idx, char in enumerate(sorted(target_chars))}
        ...
        self.max_len = max(max(len(t) for t in input_texts),
                          max(len(t) for t in target_texts)) + 2
        ...
        # Create datasets
        self.train_dataset = SpellingDataset(train_input, train_target,
                                            self.input_vocab, self.target_vocab, self.max_len)
        self.val_dataset = SpellingDataset(val_input, val_target,
                                          self.input_vocab, self.target_vocab, self.max_len)
    ...
    def build_model(self):
        ...
        encoder = Encoder(len(self.input_vocab), self.embedding_dim,
                         self.hidden_dim, self.n_layers)
        decoder = Decoder(len(self.target_vocab), self.embedding_dim,
                         self.hidden_dim, self.n_layers)
    ...
    def train(self, epochs=50, batch_size=16, lr=0.003, accumulation_steps=4, warmup_epochs=5):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (src, trg) in enumerate(pbar):
                ...
                output = self.model(src, trg, teacher_forcing_ratio=teacher_forcing_ratio)
